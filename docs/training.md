...
数据生成的耗时几乎全在 `rollout` 教师上。profiling（classic 2 人局）：
单次决策约 126ms，其中 `cloneForSearch` + `legalActions` 合计 <10%，大头是
playout 策略（heuristic）被调用约 2.7 万次/决策。决策点均值 5.4 个合法动作，
其中约 53% 价值低于最优 5 分以上，但最优与次优仅差约 4.26 分。

试过的提速方案与实测（vs `rollout` N=32，各 120~160 局）：

| 方案 | 结果 | 结论 |
|---|---|---|
| 连续减半 SH，同总预算（K×32） | 51.6% | **平价**——分配不比均匀聪明 |
| 连续减半 SH，降预算（K×16/20/24） | 33 / 43 / 38% | 明显掉 |
| 均匀降 N=24 / N=20 | 45 / 49%（分差 −3.4 / −2.6） | 略掉，近线性取舍 |

**结论：生成是蒙特卡洛计算量绑定的，没有免费午餐。** 那 53% 的「烂动作」
剪枝没能兑现成加速——大量决策是低 K（白骰阶段常 2 个动作、无可剪），而高 K
决策的 top-2 差距只有约 4 分、本就需要足额采样，SH 早轮反而按噪声误杀好臂。
故 SH 未采纳（已回退）。

可行但有取舍的杠杆：
- **降 N**：N≈20 约省 1.6×，教师略弱（分差 −2.6）。**用于蒸馏教师可接受**
  ——策略网络只需好标签、不需教师最强（`policy` 已反超 `rollout`），教师的
  小幅劣化在蒸馏中大多被洗掉；
- **并行**：引擎无共享状态，多进程/多机线性加速，是最可扩展的杠杆；
- **截断 rollout + 价值自举**（未做）：用 `policy` 的价值头估剩余、不打到终局，
  理论上能缩短 playout；但价值头 MAE≈14 分远大于 4 分的决策差距，偏差风险高、
  且耦合当前模型，需实测质量后再定。
