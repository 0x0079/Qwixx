...
| `double-a` | 同经典 | 整行单色 | 最近格可划第二叉；锁行门槛 7 | 按行（封顶 16） |
| `double-b` | 同经典 | 整行单色 | 四个双倍格；锁行门槛 7 | 按行（封顶 16） |
| `bonus-a` | 同经典 | 整行单色 | 12 个触发格 + 强制颜色轨，可连锁 | 按行 |
| `bonus-b` | 同经典 | 整行单色 | 五组成对符号与即时/终局奖励 | 按行 + 奖励 |
| `connected-steps` | 同经典 | 整行单色 | A–E 阶梯卡面，第五计分组 | 按行 + 阶梯 |
| `connected-chain` | 同经典 | 整行单色 | A–E 连锁卡面，配对格自动划记 | 按行 |
| `x-change` | 同经典 | 整行单色 | 9 个有序白骰和值交换 | 按行 |
...
注：gemixxt 预设以及 Connected 的五张数字卡面满足官方结构约束，但具体排列为数字化
重建（官方未提供机器可读卡面数据）；Double、Bonus、X-Change 的特殊格位已按官方卡面核对。
引擎另支持 `scoreBy: 'color'` 作为自定义计分模式。
